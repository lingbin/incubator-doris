...
static bool _cmp_tablet_by_creation_time(const TabletSharedPtr& a, const TabletSharedPtr& b) {
    return a->creation_time() < b->creation_time();
}

TabletManager::TabletManager() : _last_update_stat_ms(0) { }
...
OLAPStatus TabletManager::_add_tablet_unlocked(TTabletId tablet_id, SchemaHash schema_hash,
    ...
    if (exist_tablet == nullptr) {
        return _add_tablet_to_map_unlocked(tablet_id, schema_hash, tablet,
                                           update_meta, false, false);
    }
...
OLAPStatus TabletManager::_add_tablet_to_map_unlocked(TTabletId tablet_id, SchemaHash schema_hash,
    ...
    if (update_meta) {
        ...
        RETURN_NOT_OK_LOG(tablet->save_meta(), "failed to save new tablet's meta");
    }
    if (drop_old) {
    ...
    RETURN_NOT_OK_LOG(tablet->register_tablet_into_dir(),
            strings::Substitute("fail to register tablet into StorageEngine. data_dir=$0",
                                tablet->data_dir()->path()));

    _tablet_map[tablet_id].table_arr.push_back(tablet);
    _tablet_map[tablet_id].table_arr.sort(_cmp_tablet_by_creation_time);
...
OLAPStatus TabletManager::load_tablet_from_dir(DataDir* store, TTabletId tablet_id,
    ...
    RETURN_NOT_OK_LOG(TabletMeta::reset_tablet_uid(header_path),
...
OLAPStatus TabletManager::_drop_tablet_directly_unlocked(
    ...
    list<TabletSharedPtr>& candidate_tablets = _tablet_map[tablet_id].table_arr;
    ...
    while (it != candidate_tablets.end()) {
        ...
        if (!keep_files) {
            ...
            RETURN_NOT_OK_LOG(tablet->save_meta(),
                    strings::Substitute("fail to drop tablet.tablet_id=$0, schema_hash=$1",
                                        tablet_id, schema_hash));
            _shutdown_tablets.push_back(tablet);
        }
    }

    RETURN_NOT_OK_LOG(dropped_tablet->deregister_tablet_from_dir(),
            strings::Substitute("fail to unregister from root path. tablet=$0, schema_hash=$1",
                                tablet_id, schema_hash));
    return OLAP_SUCCESS;
}
