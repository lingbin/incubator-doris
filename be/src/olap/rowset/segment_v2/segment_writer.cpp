...
SegmentWriter::SegmentWriter(fs::WritableBlock* block,
                             uint32_t segment_id,
                             const TabletSchema* tablet_schema,
                             const SegmentWriterOptions& opts) :
        _segment_id(segment_id),
...
Status SegmentWriter::init(uint32_t write_mbytes_per_sec) {
    ...
    for (auto& column : _tablet_schema->columns()) {
        ...
        std::unique_ptr<ColumnWriter> writer(
                new ColumnWriter(opts, std::move(field), is_nullable, _wblock));
...
Status SegmentWriter::_write_short_key_index() {
    ...
    uint64_t offset = _wblock->bytes_appended();
    RETURN_IF_ERROR(_write_raw_data(slices));
    uint32_t written_bytes = _wblock->bytes_appended() - offset;
