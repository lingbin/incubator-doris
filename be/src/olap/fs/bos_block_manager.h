...
class BosReadableBlock;
class BosWritableBlock;
...
class BosBlockManager : public BlockManager {
 public:
  // Note: all objects passed as pointers should remain alive for the lifetime
  // of the block manager.
  BosBlockManager(Env* env, BlockManagerOptions opts);

  virtual ~BosBlockManager();
 ...
 private:
  friend class internal::BosReadableBlock;
  friend class internal::BosWritableBlock;
  ...
  // The options that the BosBlockManager was created with.
  ...
  // DISALLOW_COPY_AND_ASSIGN(BosBlockManager);
};
